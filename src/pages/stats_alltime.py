...
df = df[['title', 'artist', 'mapper_name', 'duration_seconds', 
         'upload_date', 'difficulties', 'album_released', 'sp_artist',
       ...
       'sp_popularity', 'artist_match',
       'title_match', 'duration_difference', 'match_conclusion', 'upvotes',
       'downvotes', 'score', 'last_fetched']]

df['total_votes'] = df['upvotes'] + df['downvotes']

hist_vars = ['duration_seconds', 'difficulties', 'album_released', 
        'sp_energy', 'sp_key', 'sp_loudness',
       'sp_speechiness', 'sp_acousticness', 'sp_instrumentalness',
       'sp_liveness', 'sp_valence', 'sp_tempo',
       'sp_popularity', 'upvotes',
       'downvotes', 'total_votes', 'score', 'last_fetched']
scatter_vars_x = ['top_30_artists', 'top_30_mappers', 'duration_seconds', 
         'upload_date', 'album_released', 
       'sp_danceability', 'sp_energy', 'sp_key', 'sp_loudness',
       'sp_speechiness', 'sp_acousticness', 'sp_instrumentalness',
       'sp_liveness', 'sp_valence', 'sp_tempo',
       'sp_popularity', 'upvotes',
       'downvotes', 'total_votes', 'score', 'last_fetched']
scatter_vars_y = ['duration_seconds', 
         'upload_date', 'album_released', 
       'sp_danceability', 'sp_energy', 'sp_key', 'sp_loudness',
       'sp_speechiness', 'sp_acousticness', 'sp_instrumentalness',
       'sp_liveness', 'sp_valence', 'sp_tempo',
...
if next_graph == 'Histogram - Upload Date':
    ...
    if st.button("See the Graph!"):
        if scatter_x == 'top_30_artists':
            top_artists = list(match_var['sp_artist'].value_counts(ascending=False).head(30).reset_index()['sp_artist'])
            ...
            top_mappers = list(match_var['mapper_name'].value_counts(ascending=False).head(30).reset_index()['mapper_name'])
